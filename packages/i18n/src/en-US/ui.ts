const translations = {
  ...
  settings: {
    ...
    identityVerification: {
      ...
      headerBody:
        'Prove that identify() and group() calls really come from your app. Your backend signs a JWT for each user with a signing secret, and Usertour can reject identities without a valid token.',
      headerDocs: 'Read the identity verification guide',
      secrets: {
        sectionTitle: 'Signing secret',
        generateButton: 'Generate secret',
        rotateButton: 'Rotate secret',
        rotateConfirmTitle: 'Rotate signing secret?',
        rotateConfirmDescription:
          'A new secret is generated and becomes the current one. The old secret keeps working until you revoke it — switch your backend to the new secret, watch the old one’s "Last used" time go stale, then revoke it.',
        rotateConfirmAction: 'Rotate secret',
        finishRotationTooltip:
          'A rotation is in progress. Revoke the previous secret before rotating again.',
        currentLabel: 'Current secret',
        previousLabel: 'Previous secret',
        rotatingOutBadge: 'Rotating out',
        rotatingOutHint:
          'Tokens signed with this secret are still accepted. Once your backend no longer uses it (check "Last used"), revoke it.',
        createdAt: 'Created',
        lastUsedAt: 'Last used',
        neverUsed: 'Never',
        empty: 'No signing secret yet. Generate one to start signing identity tokens.',
        copyButton: 'Copy secret',
        revealButton: 'Reveal secret',
        revokeButton: 'Revoke secret',
        revokeConfirmTitle: 'Revoke signing secret?',
        revokeConfirmDescription:
          'Tokens signed with this secret stop being accepted immediately. Make sure your backend no longer signs with it — check its "Last used" time first.',
        revokeConfirmAction: 'Revoke secret',
        revokeSuccess: 'Signing secret revoked',
        revokeFailure: 'Failed to revoke signing secret',
        createFailure: 'Failed to generate signing secret',
        dialogTitle: 'Signing secret',
        dialogCreatedTitle: 'New signing secret',
        dialogDescription:
          'Copy the secret into your backend configuration. Treat it like a password — never expose it in frontend code.',
      ...
      enforcement: {
        ...
        enableConfirmDescription:
          'Users identified without a valid identity token will be rejected and Usertour will not load for them. Make sure signed traffic reads 100% before enabling.',
        enableConfirmAction: 'Require verification',
        enabledToast: 'Identity verification is now required',
        disabledToast: 'Identity verification is no longer required',
      },
      validator: {
        title: 'Validate a token',
        description:
          'Paste an identity token generated by your backend to check it verifies against this environment’s signing secrets.',
        placeholder: 'eyJhbGciOiJIUzI1NiIs...',
        checkButton: 'Check token',
        checkFailure: 'Failed to validate the token. Please try again.',
        subject: 'User',
        company: 'Company',
        expiresAt: 'Expires',
        status: {
      ...
      coverage: {
        title: 'Signed traffic',
        description:
          'Share of identity claims over the last 7 days that carried a valid identity token. Turn on enforcement only once this reads 100%.',
        users: 'Users',
        companies: 'Companies',
        noTraffic: 'No traffic yet',
        breakdown: '{{valid}} valid · {{invalid}} invalid · {{missing}} unsigned',
